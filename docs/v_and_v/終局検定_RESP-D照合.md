...
| 反映量 | 算定 |
|---|---|
| 設計用せん断 Qmu | プッシュオーバー終局時の強軸せん断 Vy（`Qmu=上限強度倍率·|Vy|`。従来の `2·Mu/内法` を置換） |
...
- `MemberDemand` に `shear: Option<f64>`・`rp: Option<f64>` を追加。`Some` のとき Qmu と Rp を
  応答値で置き換え、`None`（既定）は従来経路（両端ヒンジ Qmu・UI 一律 Rp）を維持する。
- **簡略化（doc 兼申し送り）:** 部材別 Rp は弦回転角（弾性成分を含む）を塑性回転角の近似
  として用いる（終局時は塑性成分が支配的）。2 軸せん断の弱軸 Qmy は従来どおり `2·Muy/内法`
  を用いる（プッシュオーバーは載荷方向のみのため、直交方向せん断需要は小さい）。反映点は
...
1. **2 軸せん断の弱軸せん断需要のプッシュオーバー直接反映**。強軸 Qmu・需要曲げ・軸力・部材別
   Rp はプッシュオーバー応答から直接反映済み（§7）。2 軸せん断の弱軸せん断 Qmy は載荷直交
   方向であり、現状は両端ヒンジ略算（`2·Muy/内法`）を用いる（載荷方向のみのプッシュオーバー
   では直交方向せん断需要が小さいため簡略化）。直交方向の応答が必要な場合は両方向の
   プッシュオーバー応答の合成が課題。
